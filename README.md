...
- **Flutter SDK** `^3.10.4` — VS Code Command Palette에서 `Flutter: New Project` 선택 후 Download SDK로 설치
- **Android Studio** — Android 에뮬레이터 및 SDK Tools 설정 필요 (`flutter doctor --android-licenses` 로 라이선스 동의)
- **Xcode** (macOS 전용) — iOS 시뮬레이터 사용 시 필요, `sudo xcode-select --install` 로 커맨드 라인 도구 추가 설치
