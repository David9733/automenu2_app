...
| 기술 | 버전 | 선택 이유 |
|------|------|-----------|
| **Flutter** | 3.35.0 | Android · iOS를 단일 코드베이스로 구성하기 위해 선택<br>Hot Reload로 UI 수정 사이클을 단축하고 위젯 기반 구조로 화면 단위 개발 |
| **Dart** | 3.10.4 | 강타입 시스템으로 빌드 단계에서 오류를 사전 차단<br>`async/await` 기반 비동기 처리로 알림·초기화 흐름을 직관적으로 구성 |
| **firebase_core** | 3.15.2 | Firebase 전체 제품군의 기반 초기화 패키지로, Analytics · Crashlytics · FCM 사용을 위해 필수 포함<br>`runZonedGuarded`로 모든 비동기 영역의 미처리 예외를 Crashlytics에 자동 보고 |
| **Supabase** | 2.12.0 | PostgreSQL 기반 BaaS로 음식 데이터 관리와 조건 필터링 쿼리를 서버 설정 없이 구성 가능<br>`.eq()` 필터 조합으로 식사 시간·상황·음주 여부·카테고리 4가지 조건을 단일 쿼리로 처리 |
| **Firebase Analytics** | 11.6.0 | 추천 흐름 내 사용자 행동을 이벤트 단위로 추적하기 위해 선택<br>조건 선택·추천 결과·설정 변경 등 단계별 커스텀 이벤트로 이탈 시점 분석 가능 |
| **Firebase Crashlytics** | 4.3.10 | 앱 배포 후 런타임 오류를 원격으로 수집하고 스택 트레이스를 확인하기 위해 선택<br>비동기 예외까지 자동 보고되도록 `runZonedGuarded`로 감싸 운영 중 오류 추적 |
| **Firebase FCM** | 15.2.10 | 서버 없이 원격 푸시 알림 구조를 구성하기 위해 선택<br>향후 서버 기반 공지·마케팅 메시지 발송 확장을 고려해 포함 |
| **flutter_local_notifications** | 17.2.4 | 매일 반복되는 식사 알림을 서버 없이 기기 내에서 스케줄링하기 위해 선택<br>아침·점심·저녁 각각 ON/OFF 및 시간 설정을 `scheduleAllMealNotifications()`로 일괄 예약 |
| **timezone** | 0.9.4 | flutter_local_notifications의 정확한 반복 알림 발송을 위해 기기 시간대 처리 목적으로 선택<br>TZDateTime으로 로컬 시간 기준 스케줄링하여 시간대 오차 없이 알림 발송 |
| **Provider** | 6.1.5+1 | 테마 상태 등 앱 전역 상태를 위젯 트리 전체에 간단하게 전달하기 위해 선택<br>`Selector` 패턴으로 `themeMode` 변경 시에만 위젯 리빌드하여 불필요한 렌더링 방지 |
| **shared_preferences** | 2.5.4 | 테마·알림 시간 등 사용자 설정을 기기 로컬에 영속 저장하기 위해 선택<br>앱 재시작 후에도 설정이 유지되도록 키-값 형태로 저장 |
| **Google Mobile Ads** | 5.3.1 | AdMob 배너 광고를 앱 하단에 고정 통합하기 위해 선택<br>광고 생명주기를 위젯 생명주기와 일치시켜 간헐적 미노출 문제 해결 |
