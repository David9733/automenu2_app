...
사용자가 선택한 조건 조합을 Supabase 쿼리로 필터링하고, 예외 처리와 결과 다양성까지 FoodService 한 곳에 집중하여 구현했습니다.
...
- `_generateDynamicReasonText()`: 상황(4) × 음주(2) × 카테고리(5) 조합 템플릿에 음식 이름 해시값을 적용하여 동일 음식엔 항상 같은 문구, DB 저장 없이 맥락에 맞는 추천 이유 생성
...
다양한 모바일 기기 환경에서 UI가 깨지지 않도록 화면 방향(세로/가로)과 화면 크기에 따라 레이아웃과 UI 요소를 분기 처리했습니다.
...
- 추천 결과 단계: 추천 요청, 추천 결과 조회, 최종 음식 선택, 다른 옵션 보기, 다시 추천받기, 처음으로 돌아가기, 추천 실패
...
**깨달음**: 초기화 코드라도 "반드시 먼저 실행해야 하는 것"과 "나중에 실행해도 되는 것"을 구분하는 것이 앱 체감 속도에 직접적인 영향을 준다는 것을 몸소 느꼈습니다.
...
**깨달음**: 캐시는 단순히 성능 최적화 기법이 아니라, 사용자 입장에서 "즉각 반응하는 앱"과 "느린 앱"을 가르는 요소임을 체감했습니다. 키 설계(정렬 후 조합)처럼 사소해 보이는 부분이 캐시 적중률을 크게 좌우한다는 것도 직접 경험했습니다.
...
**깨달음**: Flutter에서 컨트롤러는 위젯에 연결되기 전까지 실제로 동작하지 않습니다. 사용 전 항상 연결 상태를 확인하고 폴백을 마련해 두는 습관이 중요하다고 느꼈습니다.
...
**깨달음**: Flutter의 위젯 빌드 사이클과 Provider 초기화 타이밍을 정확히 이해하지 않으면, 겉보기에는 정상 작동하는 것처럼 보여도 내부에서 에러가 연쇄 발생한다는 것을 실감했습니다. 비동기 초기화 로직은 항상 빌드 사이클과의 간섭을 염두에 두고 설계해야 한다는 것을 배웠습니다.
...
**체감**: 앱 개발 자체보다 배포·출시 과정에 예상치 못한 절차와 장벽이 존재한다는 것을 직접 겪으면서, 기술적 완성도만큼 출시 프로세스에 대한 이해도 중요하다는 것을 느꼈습니다.
...
**깨달음**: 데이터를 DB에 저장할지 코드로 생성할지는 단순한 구현 편의의 문제가 아니라, 유지보수 비용과 DB 설계 전략까지 연결된 결정임을 깨달았습니다. 또한 추천 결과의 품질은 필터링 로직의 세밀함에서 온다는 것을 실감했습니다.
...
**깨달음**: 기술을 선택할 때는 "할 수 있다"보다 "이 상황에 맞다"를 먼저 따져야 한다는 것을 배웠습니다. FCM도 알림 기술이지만, 서버가 없는 환경에서의 반복 알림에는 로컬 알림이 훨씬 적합한 선택이었습니다.
...
**깨달음**: 로그에 오류가 없다고 해서 정상 동작한다는 의미가 아님을 배웠습니다. 원인을 특정하기 어려운 문제일수록 패키지 버전 등 환경 변수를 먼저 점검하는 것이 효율적인 디버깅 방법임을 체감했습니다.
...
**깨달음**: 서드파티 SDK는 내 코드와 다른 생명주기로 움직입니다. 초기화 완료 시점과 위젯 생명주기를 명시적으로 맞춰 주지 않으면 간헐적 오류가 생길 수 있고, 이런 문제는 재현이 어려워 디버깅 비용이 크다는 것을 실감했습니다.
...
**깨달음**: 기능 추가 시 기존 레이아웃과의 충돌을 먼저 고려해야 한다는 것을 배웠습니다. `Stack`은 자유로운 배치가 가능하지만 UI 요소 간 공간을 자동으로 확보해 주지 않으므로, 요소들이 공간을 나눠야 하는 경우에는 `Column` 같은 흐름 기반 레이아웃이 더 적합합니다.
...
**깨달음**: 화면 회전은 개발 중에 자주 테스트하지 않는 시나리오지만, 실제 사용자는 자연스럽게 회전을 하기 때문에 반드시 검증이 필요합니다. Flutter의 위젯 재빌드 사이클을 제대로 이해하고, 컨트롤러와 비동기 작업 모두에 생명주기 가드를 일관되게 적용하는 것이 안정적인 앱의 기본임을 배웠습니다.
