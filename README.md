...
- **광고 배너와 버튼 겹침** : AdMob 배너를 기존 레이아웃 위에 단순히 올려놓자 광고 영역이 하단 버튼들 위에 겹쳐 조작을<br>방해했습니다. `Stack` 대신 `Column` 기반 레이아웃으로 구조를 변경해 광고·콘텐츠 영역이 수직으로 분리되도록 하고, 콘텐츠 영역이 광고 높이만큼 패딩을 확보하도록 조정했습니다. Stack의 터치 영역 침범은 시각적으로 드러나지 않으므로, 레이아웃 변경 시 터치 영역이 물리적으로 겹치지 않는 구조를 우선 선택해야 한다는 것을 배웠습니다.
