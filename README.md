...
#### 1. Flutter SDK 설치 (VS Code 기준)

1. VS Code를 열고 **View > Command Palette** (또는 `Ctrl + Shift + P`) 실행
2. `flutter` 입력 후 **Flutter: New Project** 선택
3. **Download SDK** 선택 후 Flutter SDK를 저장할 폴더 지정
4. **Clone Flutter** 클릭하여 설치 완료

#### 2. Android 개발 환경 설정

Android 앱 빌드 및 테스트를 위해 Android Studio가 필요합니다.

1. **Android Studio 설치** — [공식 사이트](https://developer.android.com/studio)에서 다운로드
2. **SDK Tools 설치** — Android Studio 설정에서 아래 항목 활성화
   - Android SDK Build-Tools
   - Android SDK Command-line Tools
   - Android Emulator
   - Android SDK Platform-Tools
3. **가상 기기(에뮬레이터) 생성**
   - Android Studio 우측 **Device Manager** → `+` → **Create Virtual Device**
   - 기기 크기(예: Medium Phone) 선택 후 **Finish**
   - 재생 버튼으로 에뮬레이터 실행
4. **라이선스 동의 및 환경 확인**
...
#### 3. iOS 개발 환경 설정 (macOS 전용)

> **주의:** Xcode는 macOS에서만 설치 및 사용이 가능합니다. Windows 사용자는 이 단계를 건너뛰세요.

1. **Xcode 설치** — Mac App Store에서 Xcode 검색 후 다운로드 (용량이 크므로 시간 여유를 두고 설치)
...
3. **iOS 시뮬레이터 실행** — Xcode 상단 메뉴 **Xcode → Open Developer Tool → Simulator** 선택
