...
- **동일 조건 반복 요청 시 불필요한 DB 호출** : 같은 조건으로 추천을 요청할 때마다 Supabase 쿼리가 반복 실행되었습니다. 조건 조합을 키로 하는 메모리 캐시를 구현하고 15분 유효 기간과 LRU 100개 한도를 설정했습니다. 카테고리 목록을<br>정렬 후 조합해 선택 순서가 달라도 동일 키로 캐시가 적중되도록 했습니다. 캐시는 데이터 구조보다 키 설계가 효율을 결정합니다. 의미상 동일한 요청이 입력 순서 차이로 다른 키로 처리되지 않도록 정규화를 먼저 설계해야 한다는 것을 배웠습니다.
