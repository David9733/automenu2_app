...
개발 하기 위해 프롬프트 잘 작성하는 AI(ChatGPT)입력으로 개발용 프롬프트를 만들어 갔습니다.<br>
<details>
...
```text
     ...
     │                                                                                                          │
     │ [앱 개요]                                                                                                │
     │ - 사용자가 식사 시간, 상황, 음주 여부, 카테고리를 선택하면 어울리는 음식 3개를 추천해주는 앱             │
     │ - 대상 사용자: 한국어 사용자                                                                             │
     │ - 플랫폼: Android, iOS                                                                                   │
     │                                                                                                          │
     │ [핵심 기능]                                                                                              │
     ...
     │    - 4단계: 음식 카테고리 선택 (한식 / 일식 / 양식 / 중식 / 분식) - 선택 선택사항                        │
     │    - 5단계: 추천 음식 3개 PageView로 보여주기 (좌우 스와이프)                                            │
     │                                                                                                          │
     │ 2. 알림 기능                                                                                             │
     ...
     │ - Flutter + Dart                                                                                         │
     │ - 상태관리: Provider                                                                                     │
     │ - DB: Supabase (음식 데이터 저장)                                                                        │
     │ - 인증/분석: Firebase (Analytics, Crashlytics, FCM)                                                      │
     │ - 광고: Google Mobile Ads (AdMob)                                                                        │
     │ - 로컬 저장: SharedPreferences                                                                           │
     │ - 알림: flutter_local_notifications                                                                      │
     │                                                                                                          │
     │ [Supabase DB 구조]                                                                                       │
     │ - foods 테이블: id, name, image_url, reason_text                                                         │
     │ - food_conditions 테이블: food_id, meal_time, eating_situation, drinking_status, food_category, priority │
     │                                                                                                          │
     │ 다음 내용을 도와줘:                                                                                      │
...
이를 통해 생성된 최종 프롬프트를 기반으로 AI 코딩 도구(Cursor)를 활용하여 개발을 진행했습니다.
<details>
