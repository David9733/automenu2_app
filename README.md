...
- **앱 시작 시간 지연** : Google Mobile Ads 초기화를 `runApp()` 이전에 동기로 실행하자 첫 화면 표시가 지연되었습니다. 광고·알림 초기화를 `runApp()` 이후 `.then()` 체인으로 분리해 비동기 실행으로 전환하고, Firebase Crashlytics·Supabase만 필수 선행 초기화 대상으로 유지했습니다. 초기화 코드라도 "반드시 먼저 실행해야 하는 것"과 "나중에 실행해도 되는 것"을 구분하는 것이 앱 체감 속도에 직접적인 영향을 준다는 것을 깨달았습니다.

- **동일 조건 반복 요청 시 불필요한 DB 호출** : 같은 조건으로 추천을 다시 요청할 때마다 Supabase 쿼리가 반복 실행되었습니다. 조건 조합을 키로 하는 메모리 캐시를 구현하고 15분 유효 기간과 LRU 100개 한도를 설정했습니다. 카테고리 목록을 정렬 후 조합해 선택 순서가 달라도 동일 키로 캐시가 적중되도록 했습니다. 캐시는 단순한 성능 최적화가 아니라 사용자 입장에서 "즉각 반응하는 앱"과 "느린 앱"을 가르는 요소임을 체감했습니다.

### Flutter

- **테마 로딩으로 인한 초기 렌더링 블로킹** : `ThemeProvider` 생성자에서 `_loadTheme()` 호출 시 build 단계에 `notifyListeners()` → `setState()`가 연쇄 실행되어 **"setState() called during build"** 에러가 발생했고, Firebase·Supabase·`SharedPreferences` 초기화가 한꺼번에 몰려 I/O 경합으로 테마 결정 전까지 흰 화면이 유지되었습니다. `isLoading` 상태를 추가해 로딩 중에는 기본 테마의 인디케이터를 표시하도록 분기하고, `ThemeService.getThemeMode()`에 1초 타임아웃을 설정해 저장소 접근 지연 시 라이트 모드로 즉시 폴백하도록 처리했습니다. Flutter의 빌드 사이클과 Provider 초기화 타이밍을 정확히 이해하지 않으면 겉으로는 정상처럼 보여도 내부에서 에러가 연쇄 발생할 수 있습니다.

- **화면 회전 직후 버튼 조작 시 앱 충돌** : 세로 ↔ 가로 회전 직후 버튼을 누르면 앱이 충돌하거나 응답하지 않았습니다. 위젯 트리 전체 재빌드 중 `PageController`가 `PageView`로부터 일시적으로 분리된 상태에서 `jumpToPage()`가 호출되거나, 이미 dispose된 위젯의 `setState`가 호출되는 것이 원인이었습니다. `_pageController.hasClients` 가드와 `try-catch`를 추가하고, 비동기 `setState` 호출 지점에 `if (mounted)` 체크를 적용하여 해결했습니다. 화면 회전은 개발 중 자주 테스트하지 않는 시나리오지만 실제 사용자는 자연스럽게 회전하기 때문에 반드시 검증이 필요합니다.

### 기능 설계

- **동적 추천 이유 생성 및 회식 조건 필터링** : 음식별 추천 이유를 DB 고정값으로 저장했더니 음식 수가 늘수록 DB가 비대해졌고, 조건 조합 기반 동적 생성으로 전환하는 과정에서 상황·음주 분기가 제대로 적용되지 않아 일반 반찬에 "술자리에 잘 어울립니다" 같은 어색한 문구가 출력됐습니다. 상황(4) × 음주(2) × 카테고리(5) 조합 템플릿을 분기하고 음식 이름 해시값으로 고정 선택하는 `_generateDynamicReasonText()`를 구성하고, `_isGoodForAlone()`으로 회식 조건에서 혼자 먹기 좋은 음식을 필터링했습니다. 데이터를 DB에 저장할지 코드로 생성할지는 유지보수 비용과 DB 설계 전략까지 연결된 결정임을 깨달았습니다.

- **FCM vs 로컬 알림 선택** : 매일 반복되는 식사 알림을 FCM으로 구현하려 했으나 반복 발송을 위해 별도 서버나 Cloud Functions가 필요하다는 점을 파악했습니다. `flutter_local_notifications`로 전환해 서버 없이 기기 자체에서 스케줄링하고, `NotificationService.scheduleAllMealNotifications()`로 아침·점심·저녁 알림을 기기 내에서 직접 예약하도록 구현했습니다. 기술을 선택할 때는 "할 수 있다"보다 "이 상황에 맞다"를 먼저 따져야 한다는 것을 배웠습니다.

### 광고

- **AdMob 배너 광고 간헐적 미표시** : AdMob 배너가 앱 실행 시마다 일관되지 않게 표시되거나 빈 공간으로 남았고, 오류 로그도 명확하지 않아 원인 파악이 어려웠습니다. `MobileAds.instance.initialize()` 완료 이후에 광고 로드가 시작되도록 순서를 정리하고, `BannerAdWidget`에서 광고 객체의 생명주기(load → dispose)를 위젯 생명주기와 일치하도록 관리해 안정적으로 노출되도록 했습니다. 초기화 완료 시점과 위젯 생명주기를 명시적으로 맞추지 않으면 간헐적 오류가 생길 수 있다는 것을 알게 됐습니다.

- **광고 배너와 버튼 겹침** : AdMob 배너를 기존 레이아웃 위에 단순히 올려놓자 광고 영역이 하단 버튼들 위에 겹쳐 조작을 방해했습니다. `Stack` 대신 `Column` 기반 레이아웃으로 구조를 변경해 광고·콘텐츠 영역이 수직으로 분리되도록 하고, 콘텐츠 영역이 광고 높이만큼 패딩을 확보하도록 조정했습니다. 기능 추가 시 기존 레이아웃과의 충돌을 먼저 고려해야 한다는 것을 배웠습니다.

### 디버깅

- **로컬 알림 미동작** : `flutter_local_notifications`로 전환한 뒤에도 알림이 예약은 되는 것처럼 보였지만 실제 기기에서 발송되지 않았고, 로그에도 오류가 없어 원인 파악이 어려웠습니다. Flutter 및 다른 의존성과 호환되는 버전으로 다운그레이드하면서 문제가 해결되었습니다(현재 `flutter_local_notifications: ^17.2.2`, `timezone: ^0.9.4`). 원인을 특정하기 어려운 문제일수록 패키지 버전 등 환경 변수를 먼저 점검하는 것이 효율적인 디버깅 방법임을 체감했습니다.

### 배포

- **Google Play 비공개 테스트 통과** : 정식 출시 전 일정 인원 이상이 며칠간 앱을 실행해야 하는 비공개 테스트 요건을 충족해야 했으나, 가족·지인 모집과 온라인 카페 품앗이 방식 모두 지속적인 참여를 보장하기 어려웠습니다. 비공개 테스트 대행 업체를 통해 필요 인원을 확보하고 Play Console 이상 지표도 신속히 대응해 정식 출시에 성공했습니다. 앱 개발만큼 배포·출시 과정에도 예상치 못한 절차와 장벽이 존재하며, 기술적 완성도와 함께 출시 프로세스에 대한 이해도 중요하다는 것을 체감했습니다.
