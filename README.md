...
| 기술 | 버전 | 선택 이유 |
|---|---|---|
| **Flutter / Dart** | SDK ^3.10.4 | Android · iOS를 단일 코드베이스로 구성하기 위해 선택 |
| **firebase_core** | ^3.0.0 | Firebase 전체 제품군의 기반 초기화 패키지로, Analytics · Crashlytics · FCM 사용을 위해 필수 포함 |
| **Supabase** | ^2.5.6 | PostgreSQL 기반 BaaS로 음식 데이터 관리와 조건 필터링 쿼리를 서버 설정 없이 구성 가능 |
| **Firebase Analytics** | ^11.6.0 | 추천 흐름 내 사용자 행동을 이벤트 단위로 추적하기 위해 선택 |
| **Firebase Crashlytics** | ^4.3.10 | 앱 배포 후 런타임 오류를 원격으로 수집하고 스택 트레이스를 확인하기 위해 선택 |
| **Firebase FCM** | ^15.2.10 | 서버 없이 원격 푸시 알림을 발송하는 구조를 구성하기 위해 선택 |
| **flutter_local_notifications** | ^17.2.2 | 매일 반복되는 식사 알림을 로컬에서 스케줄링하기 위해 선택 |
| **timezone** | ^0.9.4 | 기기 시간대 기반으로 정확한 알림 발송 시각을 처리하기 위해 선택 |
| **Provider** | ^6.1.1 | 테마 상태 등 앱 전역 상태를 위젯 트리 전체에 간단하게 전달하기 위해 선택 |
| **shared_preferences** | ^2.2.2 | 테마, 알림 시간 등 사용자 설정을 기기 로컬에 영속 저장하기 위해 선택 |
| **Google Mobile Ads** | ^5.1.0 | AdMob 배너 광고를 앱에 통합하기 위해 선택 |
