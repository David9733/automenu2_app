...
**깨달음**: 캐시는 단순히 성능 최적화 기법이 아니라, 사용자 입장에서 "즉각 반응하는 앱"과 "느린 앱"을 가르는 요소임을 체감했습니다.

### 3. 테마 로딩으로 인한 초기 렌더링 블로킹

**문제**: `ThemeProvider` 생성자에서 `_loadTheme()` 호출 시 build 단계에 `notifyListeners()` → `setState()`가 연쇄 실행되어 **"setState() called during build"** 에러가 발생했습니다. 동시에 Firebase·Supabase·`SharedPreferences` 초기화가 한꺼번에 몰려 I/O 경합으로 테마 결정 전까지 흰 화면이 유지되었습니다.

**개선**: `isLoading` 상태를 추가해 로딩 중에는 기본 테마의 인디케이터를 표시하도록 분기하고, `ThemeService.getThemeMode()`에 1초 타임아웃을 설정해 저장소 접근 지연 시 라이트 모드로 즉시 폴백하도록 처리했습니다.

**깨달음**: Flutter의 빌드 사이클과 Provider 초기화 타이밍을 정확히 이해하지 않으면, 겉으로는 정상처럼 보여도 내부에서 에러가 연쇄 발생할 수 있습니다.

### 4. Google Play 비공개 테스트 통과 과정

**문제**: Google Play 스토어에 앱을 정식 출시하려면 비공개 테스트(클로즈드 테스트)를 일정 인원 이상이 설치하고 며칠간 실행해야 하는 요건을 충족해야 했습니다. 처음에는 가족·지인·친구에게 부탁했으나 인원 모으기 자체가 어려웠고, 며칠씩 매일 실행을 유지해야 한다는 조건 때문에 쉽지않았습니다. 다음으로 품앗이 형태로 운영되는 온라인 카페를 이용했지만 참여자의 지속적인 실행을 보장하기 어려워 이 방법도 확실한 방법이 아니었습니다.
...
### 5. 동적 추천 이유 문구 및 회식 조건 음식 필터링 문제
...
**깨달음**: 데이터를 DB에 저장할지 코드로 생성할지는 단순한 구현 편의의 문제가 아니라, 유지보수 비용과 DB 설계 전략까지 연결된 결정임을 깨달았습니다. 

### 6. 식사 알림 구현 방식(FCM vs 로컬 알림) 선택 
...
**깨달음**: 기술을 선택할 때는 "할 수 있다"보다 "이 상황에 맞다"를 먼저 따져야 한다는 것을 배웠습니다. 

### 7. 로컬 알림이 동작하지 않는 문제

**문제**: `flutter_local_notifications`로 전환한 뒤에도 알림이 예약은 되는 것처럼 보였지만 실제로 기기에서 발송되지 않았습니다. 로그에는 오류가 없어 원인을 파악하기 어려웠습니다.

**개선**: 패키지 버전을 Flutter 및 다른 의존성과 호환되는 버전으로 다운그레이드하면서 문제가 해결되었습니다. 상위 버전에서 발생하던 호환성 문제가 원인이었던 것으로 파악됩니다. 현재는 `flutter_local_notifications: ^17.2.2`(resolved: 17.2.4)와 `timezone: ^0.9.4`를 사용하고 있으며 정상 동작하고 있습니다.

**깨달음**: 원인을 특정하기 어려운 문제일수록 패키지 버전 등 환경 변수를 먼저 점검하는 것이 효율적인 디버깅 방법임을 체감했습니다.

### 8. 구글 배너 광고가 간헐적으로 표시되지 않는 문제
...
**깨달음**:  초기화 완료 시점과 위젯 생명주기를 명시적으로 맞춰 주지 않으면 간헐적 오류가 생길 수 있는 것을 알게되었습니다.

### 9. 광고 배너가 버튼 위에 겹쳐 조작을 방해하는 문제
...
**깨달음**: 기능 추가 시 기존 레이아웃과의 충돌을 먼저 고려해야 한다는 것을 배웠습니다. 

### 10. 화면 회전 직후 버튼을 누르면 앱이 오류나거나 멈추는 문제
...
**깨달음**: 화면 회전은 개발 중에 자주 테스트하지 않는 시나리오지만, 실제 사용자는 자연스럽게 회전을 하기 때문에 반드시 검증이 필요합니다.
